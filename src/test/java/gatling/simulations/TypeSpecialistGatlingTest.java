...
/**
 * Performance test for the TypeSpecialist entity.
 */
public class TypeSpecialistGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all typeSpecialists").get("/api/type-specialists").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new typeSpecialist")
                        .post("/api/type-specialists")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"specialistType\": \"SAMPLE_TEXT\"" + ", \"stateSpecialist\": \"ACTIVE\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_typeSpecialist_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created typeSpecialist").get("${new_typeSpecialist_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created typeSpecialist").delete("${new_typeSpecialist_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TypeSpecialist entity").exec(scn);
