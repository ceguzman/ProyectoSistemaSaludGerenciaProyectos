...
/**
 * Performance test for the MedicalProcedures entity.
 */
public class MedicalProceduresGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all medicalProcedures").get("/api/medical-procedures").headers(headers_http_authenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new medicalProcedures")
                        .post("/api/medical-procedures")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"typeProcedures\": \"SAMPLE_TEXT\"" +
                                ", \"description\": \"SAMPLE_TEXT\"" +
                                ", \"dateProcedures\": \"2020-01-01T00:00:00.000Z\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_medicalProcedures_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created medicalProcedures").get("${new_medicalProcedures_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(http("Delete created medicalProcedures").delete("${new_medicalProcedures_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MedicalProcedures entity").exec(scn);
