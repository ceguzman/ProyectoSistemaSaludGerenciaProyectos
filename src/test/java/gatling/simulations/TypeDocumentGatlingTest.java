...
public class TypeDocumentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all typeDocuments").get("/api/type-documents").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created typeDocument").get("${new_typeDocument_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created typeDocument").delete("${new_typeDocument_url}").headers(headers_http_authenticated))
                .pause(10)
        );
