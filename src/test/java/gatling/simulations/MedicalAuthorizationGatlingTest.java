...
/**
 * Performance test for the MedicalAuthorization entity.
 */
public class MedicalAuthorizationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all medicalAuthorizations")
                    .get("/api/medical-authorizations")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new medicalAuthorization")
                        .post("/api/medical-authorizations")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"detailAuthorization\": \"SAMPLE_TEXT\"" +
                                ", \"stateAuthorization\": \"ACTIVE\"" +
                                ", \"dateAuthorization\": \"2020-01-01T00:00:00.000Z\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_medicalAuthorization_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created medicalAuthorization").get("${new_medicalAuthorization_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created medicalAuthorization")
                        .delete("${new_medicalAuthorization_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MedicalAuthorization entity").exec(scn);
