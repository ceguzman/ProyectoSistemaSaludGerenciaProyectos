...
/**
 * Performance test for the People entity.
 */
public class PeopleGatlingTest extends Simulation {
                    ...
                    http("Create new people")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"firstSurname\": \"SAMPLE_TEXT\"" +
                                ", \"secondName\": \"SAMPLE_TEXT\"" +
                                ", \"secondSurname\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_people_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created people").get("${new_people_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created people").delete("${new_people_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the People entity").exec(scn);
