...
/**
 * Performance test for the MedicalAppointments entity.
 */
public class MedicalAppointmentsGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all medicalAppointments")
                ...
                .exec(
                    http("Create new medicalAppointments")
                        .post("/api/medical-appointments")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"dateMedical\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_medicalAppointments_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created medicalAppointments").get("${new_medicalAppointments_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created medicalAppointments").delete("${new_medicalAppointments_url}").headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MedicalAppointments entity").exec(scn);
