 ...
 */
public class MedicationRequestGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all medicationRequests")
                ...
                .exec(
                    http("Create new medicationRequest")
                        .post("/api/medication-requests")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"name\": \"SAMPLE_TEXT\"" + ", \"amount\": 0" + ", \"milligrams\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_medicationRequest_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created medicationRequest").get("${new_medicationRequest_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(http("Delete created medicationRequest").delete("${new_medicationRequest_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MedicationRequest entity").exec(scn);
