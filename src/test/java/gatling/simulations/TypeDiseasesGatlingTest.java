...
/**
 * Performance test for the TypeDiseases entity.
 */
public class TypeDiseasesGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all typeDiseases").get("/api/type-diseases").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new typeDiseases")
                        .post("/api/type-diseases")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"diseasesType\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_typeDiseases_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created typeDiseases").get("${new_typeDiseases_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created typeDiseases").delete("${new_typeDiseases_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TypeDiseases entity").exec(scn);
